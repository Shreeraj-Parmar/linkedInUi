...
const ChooseCompanyDialog = ({
  ...
  return (
    <Dialog
      open={selectCompanyDialog}
      onClose={() => setSelectCompanyDialog(false)}
      PaperProps={{
        sx: {
          ...dialogStyle,
        },
      ...
      <div className=' w-[100%] relative h-[100%]'>
        <div className='p-3 border-b-2 border-gray-400 border-opacity-40'>
          <p className='text-[#272727] font-semibold'>
            Choose Company For New Job Post
          </p>
        </div>

        <IconButton
          style={{ position: "absolute", right: 10, top: 5 }}
          onClick={() => setSelectCompanyDialog(false)}
        >
          ...
          currUserData.company.map((com) => {
            return (
              <div
                key={com._id}
                onClick={() => {
                  setSelectCompanyForJob(com);
                  setTimeout(() => {
                    setSelectCompanyDialog(false);
                    navigate(`/job/new`);
                  }, 100);
                }}
...
export default ChooseCompanyDialog;
