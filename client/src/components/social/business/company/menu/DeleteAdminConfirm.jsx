...
import { deleteAdminFromCompany } from "../../../../../services/api.js";
...
const DeleteAdminConfirm = ({
  ...
  const handleDelete = async () => {
    let res = await deleteAdminFromCompany({
      userId: selectUserForDelete._id,
      companyId: companyId.companyId,
    });
    if (res.status === 200) {
      console.log("deleted");
      setAllAdmins((prev) => {
        return prev.filter((user) => user !== selectUserForDelete);
      });
    ...
    >
      <div className='w-[100%] h-[100%] '>
        <div className=' flex justify-between items-center p-4 border-b-2 border-gray-400 border-opacity-40'>
          <p className=' font-semibold text-xl'>Delete Admin</p>
          <button>
            <CloseIcon
              onClick={() => {
                setDeleteDialog(false);
...
export default DeleteAdminConfirm;
